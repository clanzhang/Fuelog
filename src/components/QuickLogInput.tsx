...
/**
 * 一句话快速记录组件
 * 输入/语音 → AI 解析 → 本地营养计算 → 确认卡片 → onConfirm 回调
 */
export default function QuickLogInput({
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 rounded-2xl bg-surface px-4 py-3 shadow-card">
        ...
        <button
          ...
          className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition active:scale-90 ${
            listening ? 'bg-rose-50 text-rose-500' : 'bg-primary-soft text-primary'
          }`}
          aria-label="语音输入"
        >
        ...
        <button
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary text-white transition active:scale-90 disabled:bg-ink/15"
          aria-label="发送"
        >
          {parsing ? (
      ...
      {/* 错误提示 */}
      {errorMsg && <p className="mt-2 text-center text-xs text-rose-500">{errorMsg}</p>}
